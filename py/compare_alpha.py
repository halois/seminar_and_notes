...
p = 2 #input("请输入一个素数p:")
truncOrd = 4 #input("请输入truncated次数n:")
aRange = 10 #input("请输入alpha1,2的最大范围:")

print("素数p为:",p)
print("截断次数n为:",truncOrd)
print('\n')
# 创建一个列表，用来放((\alpha_1,\alpha_2),i)
LambdaList = [((x,y),1) for x in range(1,aRange) for y in range(aRange)]+[((x,y),2) for x in range(1,aRange) for y in range(1,aRange)]
# print(LambdaList)	
