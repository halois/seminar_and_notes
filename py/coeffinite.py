...
# 比较coeffmod.py 这里直接在mod p的有限域上算，不用手工计算
from sympy import *
...
polyn = [Poly(fundpoly**i ,x,domain=FF(p)).as_expr() for i in range(1,n+1)]
...
poly_new = [1+sum(x**(j+1)*polytruncoeff[i][j] for j in range(t-1)) for i in range(n)]

print("展开后mod p的多项式为: ", polyexpand)
print("除首项1以外的系数: ",polytotalcoeff)
# print("截断到t的系数: " ,polytruncoeff)
print("截断后mod p之后的系数: " ,polytruncoeff)
