...
\end{itemize}
We list some more (blog) articles about Witt vectors, \href{https://hilbertthm90.wordpress.com/tag/witt-vectors/}{series about Witt ring},  \href{http://www.neverendingbooks.org/big-witt-vectors-for-everyone-12}{big Witt vectors for everyone}, \href{http://chromotopy.org/what-are-witt-vectors}{What are Witt vectors}.

We recommende these \href{http://www.cip.ifi.lmu.de/~grinberg/algebra/algebra.html}{sidenotes} to Hazewinkel's \emph{Witt vectors}.
\begin{figure}[htbp]
	\centering
...
\begin{definition}
	A $p$-Witt vector\index{$p$-Witt vector} over a commutative ring $R$ is a sequence $(X_0,X_1,X_2,\cdots)$ of elements of $R$.
\end{definition}
\begin{remark}
...
\begin{definition}
Truncated $p$-Witt ring\index{truncated $p$-Witt ring} $W_k(R)=\{(a_0,a_1,\cdots,a_{k-1})| a_i\in R\}$ (also called the ring of Witt vectors of length $k$).
\end{definition}
\begin{example}
...
Now we turn to the \texttt{big}(universal) Witt vectors\index{big Witt vector}. J.\,P,\ May once said ``This(the theory of Witt vectors) is a strange and beautiful piece of mathematics that every well-educated mathematician should see at least once''.
...
multiplication in $W(R)$: denoted by $*$, this is a little mysterious, we will talk the details later. For the present purposes we only define $*$ as the unique continuous functorial operation for which $(1-aT)*(1-bT)=(1-abT)$. Therefore $\prod_i (1-a_iT)*\prod_j(1-b_jT)=\prod_{i,j}(1-a_ib_jT)$.\\
...
\[R\longrightarrow W(R)=1+TR\llbracket T\rrbracket\]
\[a \mapsto [a]=1-aT\]
...
\section{Truncation set} % (fold)
\label{sec:truncation_set}
Given a truncation set $S \subset \mathbb{N}_+=\{1,2,\cdots\}$ and a commutative ring $R$ we can define a commutative ring $\mathbb{W}_S(R)$.

As a set, $\mathbb{W}_S(R)=R^S$, and we define addition and multiplication in such a way that the ghost map
\begin{align*}
w\colon \mathbb{W}_S(R) \longrightarrow& R^S\\
(a_s)_{s\in S} \mapsto& (w_s)_{s\in S}
\end{align*}
where
\[w_s=\sum_{s=di}ia_i^d.\]
This map is a ring map. One can show that there is exactly one functorial way to do this.

We will focus on three families of truncation sets.
\begin{itemize}
	\item If $S=\{1,2,3,\cdots,m\}$, we denote $\mathbb{W}_S(R)$ by $\mathbb{W}_m(R)$ and call it the big Witt vectors of length $m$.
	\item If $S=\{1,p,p^2,\cdots,p^{m-1}\}$, we denote $\mathbb{W}_S(R)$ by $\mathbb{W}_m(R;p)$ and call it the $p$-typical Witt vectors of length $m$. If $R$ is a $\mathbb{Z}_{(p)}$-algebra, there is a splitting
	\[\mathbb{W}_m(R)\cong \prod_{(p, n)=1} \mathbb{W}_{\left \lfloor \log_p(m/n)+1\right \rfloor}(R;p)\]
	\item For any $m$, there is a truncation set generated by $m$: $S=\langle m\rangle=\{d\in \mathbb{N}\mid d|m\}$ and the associated Witt vectors $\mathbb{W}_{\langle m\rangle}(R)$.
\end{itemize}
...
\[(1-rT^m)*(1-sT^n)=(1-r^{n/d}s^{m/d}T^{mn/d})^d,\ d=\mathrm{gcd}(m,n),\]
...
\paragraph{Verschiebung and Frobenius}
\cite{K-theory/0605}
Next, we introduce the Bloch-Stienstra formula on the relationship between Verschiebung and Frobenius in the context of the action of big Witt vectors on NK-groups.

For a (commutative) ring $R$ the additive group of $\mathbb{W}(R)$ is the multiplicative group $1+TR[[T]]$, $T$ a variable. The decreasing filtration of subgroups  $I_m(R) = 1+T^mR[[T]]$ makes $\mathbb{W}(R)$ a topological group and any element $w(T) \in \mathbb{W}(R)$ has a unique convergent expansion 
\[w(T) = \prod_{n\geq 1}(1 - r_nT^n), r_n \in R.\]
When $w(T) \in I_m(R)$, then the expansion is of the type 
\[w(T) = \prod_{n\geq m}(1 - r_nT^n).\] 
The multipicative structure on $\mathbb{W}(R)$ is the unique continuous extension to the whole $\mathbb{W}(R)$ of
the pairing
\[(1 - rT^m) * (1 - sT^n) = (1 - r^{n/d} s^{m/d} T^{mn/d})^d, r, s \in R, d = \mathrm{gcd}(m, n).\]
The Frobenius endomorphism is a ring endomorphism
\begin{align*}
F_m \colon \mathbb{W}(R) \longrightarrow& \mathbb{W}(R)\\
1 - rT^n \mapsto &  (1 - r^{n/d}T^{n/d})^d, d = \mathrm{gcd}(m, n), r \in R.
\end{align*}


When $\mathbb{Q}\subset R$ the ghost isomorphism between the multiplicative and additive
groups
\begin{align*}
-T\frac{ d \log}{d T}\colon 1 + TR[[T]] &\longrightarrow TR[[T]]\\
 \alpha &\mapsto  -T\frac{d \alpha}{\alpha d t}
\end{align*}
is actually a ring isomorphism $\mathbb{W}(R) \longrightarrow \prod_{1}^{\infty} R$ where the right hand side is viewed as $TR[[T]]$ under the assignment 
\begin{align*}
\prod_{1}^{\infty} R &\longrightarrow TR[[T]]\\
(r_1, r_2,\cdots) &\mapsto r_1T +r_2T^2 +\cdots.
\end{align*}
What we need here is the fact that the diagonal injection $\lambda_T \colon R \longrightarrow \prod_{1}^{\infty} R = \mathbb{W}(R)$ is invariant under
$F_m$ for all $m \in \mathbb{N}$, $F_m(\lambda_T)=\lambda_T$.

The $R$-algebra endomorphism 
\begin{align*}
R[T] &\longrightarrow R[T],\\ T  &\mapsto T^m
\end{align*}
induces a group endomorphism $V_m \colon NK_i(R) \longrightarrow NK_i(R)$ – the Verschiebung.

Bloch \cite{BLOCH1978304} and then Stienstra (Operations in the higher K-theory of endomorphisms) defined a $\mathbb{W}(R)$-module structure on $NK_i(R)$. The action of $1 - rT^m \in \mathbb{W}(R)$, $r \in R$ on $NK_i(R)$ is the effect of the composite functor
\[\mathbf{P}(R[T]) \overset{t_m}\longrightarrow  \mathbf{P}(R[T]) \overset{\bar{r}}\longrightarrow  \mathbf{P}(R[T]) \overset{v_m}\longrightarrow  \mathbf{P}(R[T])\]
where 
\begin{itemize}
	\item $v_m$ corresponds to the base change through $T \mapsto T^m$ (it gives rise to the $m$th Verschiebung),
	\item  $\bar{r}$ corresponds to the base change through $T \mapsto rT$,
	\item  and $t_n$ corresponds to the scalar restriction through $T \mapsto T^m$ (the corresponding endomorphism of $NK_i(R)$ is the transfer).
\end{itemize}
This determines the action of the whole $\mathbb{W}(R)$ because any element of $NK_i(R)$ is annihilated by the ideal $I_m(R)$ for some $m$, i.\,e.\, $NK_i(R)$ is a continuous $\mathbb{W}(R)$-module.

We have the following relationship between $F_m$ and $V_m$
\[V_m(F_m(\alpha) * z) = \alpha * z, \alpha \in \mathbb{W}(R), z \in NK_i(R), m \in N,\]
where $*$ refers to the $\mathbb{W}(R)$-module structure. 

In particular, when $\mathbb{Q}\subset R$ the equality above implies
\begin{lemma}
	For any natural number $m$ the Verschiebung $V_m$ is an $R$-linear endomorphism of $NK_i(R)$.
\end{lemma}

Weibel \cite{MR88f:18018} has generalized the $\mathbb{W}(R)$-module structure to the graded situation so that for a graded ring $A = A_0\oplus A_1\oplus \cdots$ and a subring $R \subset A_0$ there is a functorial continuous $\mathbb{W}(R)$-module structure on $K_i(A, A_+)$, where $A_+ =A_1 \oplus A_2 \oplus \cdots$.

In particular, the $\mathbb{W}(R)$-module structure on $NK_i(A, A_+)$ is the restriction of that on $NK_i(A)$ under the embedding $NK_i(A, A_+) \longrightarrow NK_i(A)$ induced by the graded homomorphism
\begin{align*}
w \colon A_0 \oplus  A_1 \oplus  A_2 \oplus  \cdots &\longrightarrow A[T] = A + TA + T^2A + \cdots,\\
a_0 \oplus  a_1 \oplus  a_2 \oplus  \cdots &\mapsto a_0 + a_1T + a_2T^2 + \cdot
\end{align*}
and, at the same time, the latter module structure is just the scalar restriction of the $\mathbb{W}(A)$-module structure through $\mathbb{W}(R) \longrightarrow \mathbb{W}(A)$. That 
\[NK_i(A, A_+) \longrightarrow NK_i(A)\]
is in fact an embedding follows from the fact that $w$ splits the non-graded augmentation $A[T] \longrightarrow A$, $T \mapsto 1$, in particular $K_i(A) \longrightarrow K_i(A[T])$ is a split monomorphism. Due to the split exact sequence 
\[0 \longrightarrow A_+ \longrightarrow A \longrightarrow A_0 \longrightarrow 0\] 
with $A$ as above, that there is a natural isomorphism $K_i(A, A_+) =K_i(A)/K_i(A_0)$. Also, the latter group will be thought of as a direct summand of $K_i(A)$ in a natural way.
